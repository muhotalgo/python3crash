...
# 단, import시 csv, tsv 파일의 인코딩은 UTF-8이어야 함!!
...
# 실행할 SQL문을 작성하고
# 실행후 결과를 커서에 저장
sql = 'select * from zipcode2013 where dong like "신대방%"'
...
# empolyees
sql = 'select * from employees where DEPARTMENT_ID like "90"'
cursor.execute(sql)
...
sql = ' select Country, Medal, count(Medal) as MedalCnt ' \
      ' from summermedals group by Country, ' \
      ' Medal order by MedalCnt Desc limit 0, 15; '
cursor.execute(sql)
...
sql = (' select Country, Medal, count(Medal) as MedalCnt from summermedals '
       ' group by Country, Medal order by MedalCnt Desc; ')
cursor.execute(sql)
...
# 사원 이름, 성, 직책, 연봉, 부서명 조회
conn = sqlite3.connect('db/bigdata.db')

cursor = conn.cursor()

sql = ' select first_name, last_name, job_id, salary, department_name ' \
      ' from employees inner join main.departments d ' \
