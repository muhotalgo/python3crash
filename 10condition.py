...
# 조건식이 참일때 값 if 조건식 else 조건이 거짓일 때 값

# 수입, 지출금액을 입력받아 흑자/적자 여부 출력
...
isTake = input('다른 지원금 수령 여부? (예:1, 아니요:2)')
isTarget = income <= 4000000 and isTake == '2'

result = '수급대상자' if isTarget else '수급비대상자'
...
wktm = int(input('근무시간을 입력하세요. '))
haru = op.mul(3, 8)
import math
print(math.ceil(24 / wktm))
...
# 어느 것이 더 싼지 알아보세요
# 단, 2022.02.04 기준 환율을 적용하세요

dtow = 780 * 1198.3522
etow = 650 * 1372.0514
cond = op.lt(dtow, etow)

result = '달러가 더 싸다' if cond else '유로가 더 싸다'
print(result)
...
result = '달러 노트북 비쌈' if dlrnb > errnb else '유로 노트북 비쌈'
